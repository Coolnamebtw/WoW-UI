...
local function RestackBagCheck( loc_id, bag_id )
	...
	if bag_id == REAGENTBANK_CONTAINER and not ArkInventory.ClientIsReagentBankUnlocked( ) then
...
local function Consolidate( loc_id )
	...
	if loc_id == ArkInventory.Const.Location.Bank then
		...
		if ArkInventory.db.option.restack.deposit and ArkInventory.ClientIsReagentBankUnlocked( ) then
...
--[[
	...
	if loc_id == ArkInventory.Const.Location.Bank then
		...
		if ArkInventory.Global.Mode.Bank then
			...
			if ArkInventory.ClientCheck( WOW_PROJECT_MAINLINE ) and ArkInventory.db.option.restack.blizzard then
