...
function ArkInventory.ScanBag_Threaded( blizzard_id, loc_id, bag_id, thread_id )
	...
	if loc_id == ArkInventory.Const.Location.Bank then
		...
		if blizzard_id == REAGENTBANK_CONTAINER then
			...
			if ArkInventory.ClientIsReagentBankUnlocked( ) then
				texture = ArkInventory.Global.Location[loc_id].Texture
				status = ArkInventory.Const.Bag.Status.Active
			else
				count = 0
...
function ArkInventory.ScanAuction_Threaded( blizzard_id, loc_id, bag_id, thread_id, massive )
	if ArkInventory.Const.TOC >= 80300 then
...
function ArkInventory.ScanProfessions( )
	...
	local p = { ArkInventory.ClientGetProfessions( ) }
	--ArkInventory.Output( "skills active = [", p, "]" )
	
	local info = ArkInventory.GetPlayerInfo( )
	info.skills = info.skills or { }
	--ArkInventory.Output( "skills saved = [", info.skills, "]" )
	
	local changed = false
	
	for index = 1, ArkInventory.Const.Skills.numPrimary + ArkInventory.Const.Skills.numSecondary do
		
		if p[index] then
			
			local name, texture, rank, maxRank, numSpells, spelloffset, skillLine, rankModifier = ArkInventory.ClientGetProfessionInfo( p[index] )
			
			if info.skills[index] ~= skillLine then
				changed = true
				info.skills[index] = skillLine
				--ArkInventory.Output( "learned [", index, "] = [", skillLine, "] [", name, "]" )
			end
			
		else
			
			if info.skills[index] ~= nil then
				changed = true
				info.skills[index] = nil
				--ArkInventory.Output( "unlearned [", index, "] )
			end
...
function ArkInventory.CollectProfessionData( )
	
	ArkInventory.Output( "CollectProfessionData" )
	
	if not ArkInventory.ClientCheck( WOW_PROJECT_MAINLINE ) then
		ArkInventory.Output( "aborting - can only run on retail" )
		return
	end
	
	local info = ArkInventory.GetPlayerInfo( )
	if not info.skills then
		ArkInventory.Output( "aborting - player skill data not found" )
		return
	end
	
	ArkInventory.Output( "skills = ", info.skills )
	
	for k, v in pairs( info.skills ) do
		
		ArkInventory.Output( "skill [", k, "] = [", v, "] = [ ", ArkInventory.Const.Skills.Data[v].text, "]" )
