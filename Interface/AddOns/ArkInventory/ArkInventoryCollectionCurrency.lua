...
local function ScanInit( )
	...
	for id = 1, 5000 do
		...
		local name, currentAmount, icon, currentWeeklyAmount, weeklyLimit, maximumAmount, isDiscovered, quality = GetCurrencyInfo( id )
...
local function Scan_Threaded( thread_id )
	...
	for index = 1, GetCurrencyListSize( ) do
		...
		if currencyID and not isHeader and name and name ~= "" then
			...
			local name, currentAmount, icon, currentWeeklyAmount, weeklyLimit, maximumAmount, isDiscovered, quality = GetCurrencyInfo( i )
