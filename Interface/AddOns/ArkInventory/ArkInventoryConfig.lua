...
function ArkInventory.ConfigInternalCategoryRuleAdd( name )
	...
	if p == -1 then
		ArkInventory.OutputError( string.format( ArkInventory.Localise["CONFIG_LIST_ADD_LIMIT_TEXT"], ArkInventory.Localise["RULES"] ) )
		return
	end
...
function ArkInventory.ConfigInternalAccountAdd( name )
	...
	if p == -2 then
		ArkInventory.OutputError( string.format( ArkInventory.Localise["CONFIG_LIST_ADD_UPGRADE_TEXT"], ArkInventory.Localise["ACCOUNT"] ) )
		return
	end
