...
--	["SetText"] = true,
	...
	["SetItemKey"] = ArkInventory.ClientSet( WOW_PROJECT_MAINLINE ),
	["SetAuctionItem"] = ArkInventory.ClientSet( WOW_PROJECT_CLASSIC ),
	["SetAuctionSellItem"] = ArkInventory.ClientSet( WOW_PROJECT_CLASSIC ),
	["SetBagItem"] = true,
	["SetBackpackToken"] = ArkInventory.ClientSet( WOW_PROJECT_MAINLINE ),
	["SetBuybackItem"] = true,
	["SetCurrencyByID"] = ArkInventory.ClientSet( WOW_PROJECT_MAINLINE ),
	["SetCurrencyToken"] = ArkInventory.ClientSet( WOW_PROJECT_MAINLINE ),
	["SetCurrencyTokenByID"] = ArkInventory.ClientSet( WOW_PROJECT_MAINLINE ),
	["SetCraftItem"] = ArkInventory.ClientSet( WOW_PROJECT_CLASSIC ),
	["SetCraftSpell"] = ArkInventory.ClientSet( WOW_PROJECT_CLASSIC ),
	["SetGuildBankItem"] = ArkInventory.ClientSet( WOW_PROJECT_MAINLINE ),
	["SetHeirloomByItemID"] = ArkInventory.ClientSet( WOW_PROJECT_MAINLINE ),
	["SetHyperlink"] = true,
	["SetInboxItem"] = true,
	...
	["SetQuestCurrency"] = ArkInventory.ClientSet( WOW_PROJECT_MAINLINE ),
	["SetQuestItem"] = true,
	["SetQuestLogCurrency"] = ArkInventory.ClientSet( WOW_PROJECT_MAINLINE ),
	["SetQuestLogItem"] = true,
	["SetQuestLogRewardSpell"] = true,
	["SetQuestLogSpecialItem"] = ArkInventory.ClientSet( WOW_PROJECT_MAINLINE ),
	["SetQuestRewardSpell"] = true,
	["SetRecipeReagentItem"] = ArkInventory.ClientSet( WOW_PROJECT_MAINLINE ),
	["SetRecipeResultItem"] = ArkInventory.ClientSet( WOW_PROJECT_MAINLINE ),
	["SetSendMailItem"] = true,
	["SetToyByItemID"] = ArkInventory.ClientSet( WOW_PROJECT_MAINLINE ),
	["SetTradePlayerItem"] = true,
	["SetTradeSkillItem"] = ArkInventory.ClientSet( WOW_PROJECT_CLASSIC ),
	["SetTradeTargetItem"] = true,
--	["SetUnit"] = false, --  > conflicts with OnSetUnit, do NOT use
	["SetVoidItem"] = ArkInventory.ClientSet( WOW_PROJECT_MAINLINE ),
	["SetVoidDepositItem"] = ArkInventory.ClientSet( WOW_PROJECT_MAINLINE ),
	["SetVoidWithdrawalItem"] = ArkInventory.ClientSet( WOW_PROJECT_MAINLINE ),
...
function ArkInventory.TooltipSetHyperlink( tooltip, h )
	...
	if h then
		...
		elseif osd.class == "battlepet" then
			--return ArkInventory.TooltipBattlepetBuild( tooltip, h )
		else
			--ArkInventory.OutputWarning( "attempting to set hyperlink for class [", osd.class, "] = ", h )
		end
...
--		ArkInventory.Output2( "[", bp_SpeciesID, "] [", bp_Level, "] [", bp_BreedQuality, "] [", bp_MaxHealth, "] [", bp_Power, "] [", bp_Speed, "] [", bp_Name, "]" )
		...
		ArkInventory.TooltipBattlepetBuild( tooltip, h )
...
function ArkInventory.TooltipIsReady( tooltip )
	...
	if txt and txt ~= "" and txt ~= ArkInventory.Localise["TOOLTIP_NOT_READY"] then
		return true
	end
end
...
function ArkInventory.HookTooltipSetQuestItem( tooltip, ... )
	
	if checkAbortItemCount( tooltip ) then return end
	
	local fn = "SetQuestItem"
	
	ArkInventory.SaveTooltipOnUpdateData( tooltip, fn, ... )
	
	local arg1, arg2 = ...
	if arg1 and arg2 then
		local name, h = tooltip:GetItem( )
		if h then
			ArkInventory.TooltipAddItemCount( tooltip, h )
		end
	end
...
--	checked ok - 3.08.
	...
	if ( i and numOwned > 1 ) or ( not i and numOwned > 0 ) then
		...
		for k, pd in pairs( tt ) do
			...
			info = string.format( "%s%s%s|r%s", info, c, _G[string.format( "ITEM_QUALITY%d_DESC", pd.quality )], "|TInterface\\PetBattles\\PetBattle-StatIcons:0:0:0:0:32:32:16:32:0:16|t" )
...
function ArkInventory.TooltipBattlepetBuild( tooltip, h, i )
	
	if not tooltip then return true end
	if not ArkInventory:IsEnabled( ) then return true end
	if not h then return end
	
	--ArkInventory.Output( "0 - TooltipBattlepetBuild" )
	...
	if sd.isWild then
		name = string.format( "%s%s|r", select( 5, ArkInventory.GetItemQualityColor( quality ) ), name )
	end
	...
	if sd.canBattle then
		...
		local txt1 = ArkInventory.Localise["TYPE"]
		local txt2 = _G[string.format( "BATTLE_PET_NAME_%s", sd.petType )] or ArkInventory.Localise["UNKNOWN"]
		...
		if ( not sd.isWild ) and ( quality ~= -1 ) then
			...
			local c = select( 5, ArkInventory.GetItemQualityColor( quality ) )
			tooltip:AddDoubleLine( PET_BATTLE_STAT_QUALITY, string.format( "%s%s %s", c, _G[string.format( "ITEM_QUALITY%d_DESC", quality )], "|TInterface\\PetBattles\\PetBattle-StatIcons:0:0:0:0:32:32:16:32:0:16|t" ) )
		end
...
function ArkInventory.TooltipBattlepetShow( tooltip, h, i )
	
	if checkAbortShow( tooltip ) then return true end
	
	if not h then return end
	
	--ArkInventory.Output( "0 - TooltipBattlepetShow" )
	
	ArkInventory.TooltipDataReset( tooltip )

	-- mouseover pet items, and clicking on pet links
	-- unit mouseover tooltip for wild and player pets is done at HookTooltipOnSetUnit, not here
	...
	ArkInventory.TooltipBattlepetBuild( tooltip, h, i )
	
	tooltip:Show( )
	
	ArkInventory.TooltipAddBattlepetDetail( tooltip, osd.id, i )
	
	ArkInventory.API.CustomBattlePetTooltipReady( tooltip, h, unpack( osd ) )
...
--	ArkInventory.Output2( "0 - HookBattlePetToolTip_Show" )
...
function ArkInventory.HookTooltipOnSetItem( tooltip, ... )
...
function ArkInventory.HookTooltipOnSetSpell( tooltip, ... )
...
function ArkInventory.ReloadTooltipOnSetUnit( tooltip, ... )
	...
	if C_PetJournal and arg1 and ( UnitIsWildBattlePet( arg1 ) or UnitIsOtherPlayersBattlePet( arg1 ) or UnitIsBattlePetCompanion( arg1 ) ) then
