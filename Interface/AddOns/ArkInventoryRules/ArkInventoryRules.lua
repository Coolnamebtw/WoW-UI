...
$Revision: 2592 $
$Date: 2020-05-24 13:14:55 +1000 (Sun, 24 May 2020) $
	...
	if IsAddOnLoaded( "Scrap" ) then
		...
		if IsAddOnLoaded( "Scrap_Merchant" ) then
			if Scrap.ToggleJunk then
				...
				if ArkInventory.db.option.message.rules.hooked then
					ArkInventory.Output( string.format( "%s: Scrap Merchant %s", ArkInventory.Localise["RULES"], ArkInventory.Localise["ENABLED"] ) )
				end
...
function ArkInventoryRules.AppliesToItem( i )
	...
	if i.h then
		...
		if not ArkInventory.TooltipIsReady( ArkInventory.Global.Tooltip.Scan ) then
			--ArkInventory.Output2( i.h, " not ready" )
			return nil
		end
	end
...
function ArkInventoryRules.System.boolean_tooltip( ... )
	...
	if ArkInventoryRules.Object.test_rule then
		...
		ArkInventory.TooltipSetItem( ArkInventoryRules.Tooltip, ArkInventoryRules.Object.loc_id, ArkInventoryRules.Object.bag_id, ArkInventoryRules.Object.slot_id, ArkInventoryRules.Object.h, ArkInventoryRules.Object )
--		if loc_id == ArkInventory.Const.Location.Vault and ArkInventory.Global.Mode.Vault then
--			ArkInventory.TooltipDump( ArkInventoryRules.Tooltip )
--		end
	end
